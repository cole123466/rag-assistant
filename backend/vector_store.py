...
class VectorStore:
    ...
    def get_lesson_link(self, course_title: str, lesson_number: int) -> Optional[str]:
        """Get the link for a specific lesson in a course"""
        import json
        
        try:
            # Get course metadata from catalog
            course_data = self.course_catalog.get(ids=[course_title])
            
            if not course_data or not course_data['metadatas'] or not course_data['metadatas'][0]:
                return None
                
            # Extract lessons metadata from JSON string
            lessons_json = course_data['metadatas'][0].get('lessons_json')
            if not lessons_json:
                return None
                
            lessons = json.loads(lessons_json)
            
            # Find the specific lesson by number
